...
class ContinualSemSegEvaluator(DatasetEvaluator):
    ...
    def evaluate(self):
        ...
        * Pixel Accuracy (pACC)
        ...
        macc_base = np.sum(acc[1:self.base_classes]) / (self.base_classes - 1)
        macc_old = np.sum(acc[1:self.old_classes]) / (self.old_classes - 1)
        macc_new = np.sum(acc[self.old_classes:]) / self.new_classes
        macc_novel = np.sum(acc[self.base_classes:]) / self.novel_classes if self.novel_classes > 0 else 0.
        ...
        res["mACC"] = 100 * macc
        res["mACC_base"] = 100 * macc_base
        res["mACC_old"] = 100 * macc_old
        res["mACC_new"] = 100 * macc_new
        res["mACC_novel"] = 100 * macc_novel
