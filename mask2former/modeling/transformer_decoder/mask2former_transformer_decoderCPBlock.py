...
class GPtAttentionLayer(nn.Module):
    ...
    def forward_post(self, tgt,prompt,
                     tgt_mask: Optional[Tensor] = None,
                     tgt_key_padding_mask: Optional[Tensor] = None,
                     query_pos: Optional[Tensor] = None):
        ...
        if prompt is None:
            ...
            q = k = self.with_pos_embed(tgt, query_pos)
            k_prompt=prompt[:int(prompt.shape[0]/2),:,:]
            v_prompt=prompt[int(prompt.shape[0]/2):,:,:]
            k=torch.cat((k, k_prompt), dim=0)
            tgt_prompt=torch.cat((tgt, v_prompt), dim=0)
            tgt2 = self.self_attn(q, k, value=tgt_prompt, attn_mask=tgt_mask,
                                key_padding_mask=tgt_key_padding_mask)[0]
    ...
    def forward(self, tgt,prompt,
        ...
        return self.forward_post(tgt, prompt, tgt_mask,
                                 tgt_key_padding_mask, query_pos)
...
class MultiScaleMaskedTransformerDecoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.transformer_GP_attention_layers = nn.ModuleList()
        for _ in range(2):
            self.transformer_GP_attention_layers.append(
                GPtAttentionLayer(
                    d_model=hidden_dim,
                    nhead=nheads,
                    dropout=0.0,
        ...
        if num_prompts > 0:
            ...
            self.g_prompt_ema = nn.ModuleList([
                nn.Embedding(20, hidden_dim),
                nn.Embedding(20, hidden_dim)
            ])
            for embedding in self.g_prompt_ema:
                embedding.weight.requires_grad_(False)
            for ema_param, param in zip(self.g_prompt_ema, self.g_prompt):
                ema_param.weight.data.copy_(param.weight.data)
    ...
    def update_ema(self):
        for ema_param, param in zip(self.g_prompt_ema, self.g_prompt):
            param.weight.data = self.ema_momentum * ema_param.weight.data + \
                                   (1 - self.ema_momentum) * param.weight.data
    ...
    def forward_base_train(self, x, src, pos, size_list, mask_features):
        ...
        g_prompt=[self.g_prompt[i].weight.unsqueeze(1).repeat(1, bs, 1) for i in range(len(self.g_prompt))]
        ...
        for i in range(self.num_layers):
            ...
            if i==0 or i==1:
                src[level_index] = self.transformer_GP_attention_layers[i](
                    src[level_index],g_prompt[i], tgt_mask=None,
    ...
    def forward_prediction_heads(self, output, class_embed, mask_embed, mask_features, 
        ...
        outputs_class = class_embed(decoder_output) 
