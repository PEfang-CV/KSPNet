...
class MaskFormer(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        features,loss_cisSum = self.backbone(images.tensor)
        ...
        if self.training:
            if self.criterion is None:  
                return {"features": features, "outputs": outputs, "shape": images.tensor.shape[-2:],"loss_cis":loss_cisSum}
            else:
                if self.per_pixel:
...
#         scores = scores[keep]
    ...
    def freeze_for_prompt_tuning(self,cfg):
        """
        freeze parameters for visual prompt tuning
        """
        if not self.model_old:
            ...
            for k, p in self.sem_seg_head.predictor.g_prompt.named_parameters():
                p.requires_grad = True
 
            for k, p in self.backbone.named_parameters():
                if "lora_adapters."+str(cfg.CONT.TASK) in k:
                    p.requires_grad = True
