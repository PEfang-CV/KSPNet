...
from detectron2.data.transforms import Transform, ColorTransform
...
def AlbumentationsWrapper():
    albumentations_augmentations = A.Compose([
        ...
        A.ToGray(p=0.2),  
        A.ElasticTransform(alpha=30, sigma=5, alpha_affine=5, p=0.5), 
    ])
    return AlbumentationsTransform(albumentations_augmentations)
...
class MaskFormerSemanticDatasetMapper:
    ...
    def from_config(cls, cfg, is_train: bool = True):
        ...
        if is_train:
            ...
            augs.append(T.RandomRotation([-15, 15]))  
            augs.append(T.RandomBrightness(0.7, 1.3)) 
            augs.append(T.RandomContrast(0.7, 1.3))  
            augs.append(T.RandomSaturation(0.7, 1.3))  
            augs.append(T.RandomLighting(0.1))  
